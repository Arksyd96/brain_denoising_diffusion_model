...
class EncodingBlock(nn.Module):
    def __init__(self, in_channels, out_channels, temb_dim, downsample=True, attn=False, num_blocks=2, groups=8) -> None:
        super().__init__()
...
class DecodingBlock(nn.Module):
    def __init__(self, in_channels, out_channels, temb_dim, upsample=True, attn=False, num_blocks=2, groups=8) -> None:
        super().__init__()
        self.resnet = nn.ModuleList([
            ResidualBlock(in_channels if i == 0 else out_channels, out_channels, temb_dim=temb_dim, groups=groups)
            for i in range(num_blocks)
...
class ResUNet(nn.Module):
    def __init__(self, in_channels, T=1000) -> None:
        super().__init__()
...
class DiffusionModule(nn.Module):
    ...
    def sample(self, model, x_T):
        ...
        for timestep in tqdm(range(self.T - 1, -1, -1), desc='Sampling', position=0, leave=True):
            ...
            predicted = self.reverse_process(model, x_t[:, 0, None], times)
